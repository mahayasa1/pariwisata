...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Simpan ke tabel tb_sponsorship
        $query1 = mysqli_query($conn, "INSERT INTO tb_sponsor (nama, alamat, no_telp, email, username, password)
        ...
        VALUES ('$username', '$password', 'sponsor')");

        if ($query1 && $query2) {
            mysqli_commit($conn);
            echo "<script>alert('Pendaftaran berhasil!'); window.location='../login/login_sponsorship.php';</script>";
        } else {
            throw new Exception("Gagal insert ke salah satu tabel.");
        }
    } catch (Exception $e) {
        mysqli_rollback($conn);
        echo "<script>alert('Pendaftaran gagal!'); window.location='register_sponsorship.php';</script>";
    }
}
?>
            ...
            <div class="row justify-content-center">
                <div class="col-md-6 col-lg-5">
                    <form method="POST" action="" class="p-4 bg-dark rounded">
                        <h3 class="fw-normal mb-4 text-center" style="letter-spacing: 1px;">REGISTER SPONSORSHIP</h3>
                        ...
                        <p class="text-center">Sudah punya akun? <a href="../login/login_sponsorship.php" class="link-info">Masuk di sini</a></p>
                    </form>
                </div>
            </div>
        </div>
